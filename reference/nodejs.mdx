...
| `info` | `() => Promise<JsBoxInfo>` | async | Get info. Rejects with `Box not yet created. Call exec() first.` if the box does not exist yet |
...
| Method | Signature | Notes |
|---|---|---|
| `info` | `() => Promise<JsBoxInfo>` | async |
| `exec` | `(command, args?, env?, tty?, user?, timeoutSecs?, workingDir?) => Promise<JsExecution>` | Execute a command |
...
State lives in `JsBoxStateInfo`: `{ status: string; running: boolean; pid?: number }`. Correct access: `(await box.info()).state.status`.
...
Cause: the box is created lazily. The synchronous `box.id` getter is unavailable before creation, and `info()` — which returns a `Promise`, like every Node accessor here — rejects. Use `await box.getId()` / `await box.getInfo()`, which create the box first, or run one command before reading either.
