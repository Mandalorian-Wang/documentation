...
```bash
python3 -m pip install 'boxlite[sync]'
```
...
| `volumes` | `List[Tuple[str, str, bool]]` | `[]` | Volume mounts as (host_path, guest_path, read_only) |
...
Synchronous wrappers using greenlet fiber switching. Requires `python3 -m pip install 'boxlite[sync]'`.
...
- [Getting Started](/getting-started/install) - Installation and quickstart guide
