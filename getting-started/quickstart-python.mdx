...
> `Boxlite` is entered with a **synchronous** `with`, and the `Boxlite.default()` factory is synchronous too. Its instance methods (`list_info()` / `create()` / `get()` / `remove()` / `metrics()` / `shutdown()`) are **async — always `await` them**, or you get `RuntimeError: no running event loop`. `box.info()` must be awaited too.
