...
**Requirements:** Rust toolchain plus a local BoxLite source checkout.

<Warning>
This docs update does not claim a verified published crate install path. A remote registry check was not available during verification, so this page keeps the Rust quickstart anchored to repository-backed source and API examples.
</Warning>
