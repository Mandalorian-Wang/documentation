    ...
    try:
        with boxlite.Boxlite.default() as runtime:
            ...
            for info in await runtime.list_info():
                # both box.info() and runtime.list_info() must be awaited
...
> (requires `await`)**. `box.info()` must be awaited too, as must
> `runtime.list_info()` /
...
| `RuntimeError: no running event loop` on a runtime call | `Boxlite` is entered with a synchronous `with`, but its methods are async | `await` every runtime method, and `box.info()` as well |
