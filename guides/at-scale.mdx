...
> (requires `await`)**. Note the distinction: `box.info()` is synchronous on
> the native `Box` and a coroutine on the wrapper types, whereas `runtime.list_info()` /
...
| `RuntimeError: no running event loop` on a runtime call | `Boxlite` is entered with a synchronous `with`, but its methods are async | `await` every runtime method. `box.info()` is synchronous only on the native `Box` |
