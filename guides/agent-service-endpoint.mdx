...
The outer object is `BoxInfo.state` (a `BoxStateInfo`), and the inner field is `status` (a string). Both levels must be written correctly — the Node equivalent, `info.state.status`, is already shown in the Quick Example above.
