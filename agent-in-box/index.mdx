...
async def main():
    ...
    try:
        ...
        async with SkillBox() as skill_box:
            # id is a property; info() is a coroutine on SimpleBox and its subclasses
...
| `info` | `info()` | `BoxInfo` | **Coroutine on `SkillBox`** (a `SimpleBox` subclass) — `await box.info()`. Synchronous only on the native `Box` |
...
`info()` touches no VM. On `SkillBox` and every other `SimpleBox` subclass it is a coroutine, so write `info = await box.info()`; calling it without `await` yields `AttributeError: 'coroutine' object has no attribute 'state'`. Only the native `Box` is synchronous. Access state through `info.state.status`.
