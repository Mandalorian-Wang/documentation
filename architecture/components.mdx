...
- `BoxManager` tracks boxes and their lifecycle state.
- `ImageManager` owns OCI image caching and retrieval.
- `FilesystemLayout` defines the runtime directory structure under `~/.boxlite`.
- `InitRootfs` provides the shared init rootfs used by guests.
- `RuntimeMetrics` uses atomic counters for low-overhead metrics.
...
- The ready notification arrives after the guest agent connects over vsock on port `2696`.
- `Guest.Init` prepares mounts, rootfs, and networking.
- `Container.Init` configures the OCI container.
- `Execution.Exec` starts command execution and begins streaming output.
