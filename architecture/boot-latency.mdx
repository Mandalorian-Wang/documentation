...
| `await box.info()` errors, or `box.info()` returns a coroutine | `info()` is synchronous on the native `Box` and a coroutine on `SimpleBox` and its subclasses | Native `Box`: `box.info()`. Wrapper types: `await box.info()`. The status is in `info.state.status`. |
