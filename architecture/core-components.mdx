...
- **Key fact**: `info()` is synchronous on the native `Box` and a coroutine on `SimpleBox` and its subclasses. Either way the returned `BoxInfo` uses `.state` (of type `BoxStateInfo`), and the status string is in `BoxStateInfo.status`.
