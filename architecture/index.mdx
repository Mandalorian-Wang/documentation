...
async def main() -> None:
    ...
    try:
        async with SimpleBox(image="alpine:latest") as box:
            # 2) box.info() is a synchronous method (do not await). state is a BoxStateInfo; the field is named status.
            info = box.info()
...
The equivalent Node implementation (showing the same chain):
...
| Read Box state | `box.info().state.status` (`info()` is synchronous; outer `state`, inner field `status`) | `box.info().state.status` (`info()` is synchronous) |
...
| `info()` | **sync** | Return `BoxInfo`, **do not await** |
...
```python
info = box.info()                 # info() is a synchronous method; do not await
