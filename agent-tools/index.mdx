...
`info()` touches no VM, but its shape depends on the type. On `SimpleBox` and its subclasses it is a coroutine: `info = await box.info()`. On the native `Box` it is synchronous, and awaiting it raises `TypeError: object BoxInfo can't be used in 'await' expression`. Read state via `info.state.status`.
