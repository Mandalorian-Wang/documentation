...
Read the current state through `info()`, which touches no VM. **Whether to await it depends on the type**: on the `Box` returned by `runtime.create()` it is synchronous, while on `SimpleBox` and its subclasses it is a coroutine. Field names are exact and easy to get wrong: see [`BoxInfo` / `BoxStateInfo`](#boxinfo-boxstateinfo-state-access).
...
`info()` differs between the two layers, and each one fails differently when you guess wrong.

```python
# Box, from runtime.create() — synchronous
info = box.info()
# await box.info()  ->  TypeError: object BoxInfo can't be used in 'await' expression

# SimpleBox and its subclasses — a coroutine, so it must be awaited
info = await box.info()
# box.info().state  ->  AttributeError: 'coroutine' object has no attribute 'state'
