...
| **Stopped** | `stop()`, or the init process exiting | VM released, **rootfs and metadata kept** | Still found by `get()` / `list_info()`; `exec` or `start()` brings it back with files intact |
...
|---|---|
| `stop()` | You call it |
| **The init process exits** | Your `entrypoint + cmd` finishes or crashes |
| `auto_stop` | Idle timeout, on BoxLite Cloud — see [Box lifecycle](/cloud/box-lifecycle) |

`entrypoint + cmd` runs as the container init, PID 1 inside the box. When it exits, the box stops. An init of `sh -c 'echo "pid=$$"; exit 7'` prints `pid=1` and the status reads `stopped` about a second later.

That gives you two rules:

- **Once init has exited, `exec` is refused**: `RuntimeError: invalid state: Cannot exec box <id>: it is stopped, and its main command has already exited`. Restart with `start()`.
- **You cannot stop init from inside the box.** `kill -TERM 1` is dropped, because PID 1 in a PID namespace ignores signals it has no handler for. That is kernel behaviour, not a BoxLite bug. Init ends when it exits on its own, or when the host calls `stop()`.

Status flips about a second after init exits. Poll it rather than reading it immediately.

For how `entrypoint` and `cmd` combine, see [Environment and startup](/manage-sandbox/environment#the-relationship-between-entrypoint-and-cmd).
...
|---|---|---|---|
| **Neutral init, then `exec`** | Keep the image's init; run jobs with `exec`, services with `nohup … &` | Independent of the work | A crashed service leaves the box `running` and the port dead |
| **Work as init** | Set `entrypoint` / `cmd`, then `attach()` | Bound to the work | Requires `runtime.create()`; `SimpleBox` has no `attach()` |
| **Detached init** | Work as init, plus `detach=True` and `auto_delete=0` | Outlives the process that created it | You remove it yourself |

Most agent work wants the first pattern: the agent is a job, and you still want to read the workspace after it finishes. A long-running service usually wants the second, so that a dead service means a dead box instead of a silent one.

### Work as init, and reading its exit code

`BoxStateInfo` carries `pid`, `running` and `status` — **no exit code**. `attach()` with no arguments follows the init session, and its `wait()` is the only way to get that code.
...
async def main() -> None:
    ...
    execution = await box.attach()          # no argument: follow the init session
    async for line in execution.stdout():
        print(line.rstrip())                # -> work done

    result = await execution.wait()
    print("init exit code:", result.exit_code)   # -> 7
...
`attach()` is async-only; the sync API has no equivalent.
...
Its init process exited. `exec` then fails with `invalid state: Cannot exec box <id>: it is stopped, and its main command has already exited`. Either make init long-running, or accept the box as single-shot and read the exit code with `attach()`. See [What stops a box](#what-stops-a-box).

### `kill -TERM 1` leaves the service running
