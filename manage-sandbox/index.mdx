...
| `Box` / `SimpleBox` / `CodeBox`, etc. | Operate a single Box (exec, copy, stop)    | **Asynchronous** `async with` | Yes, `await` — `info()` included                                                                                                                            |

Key point: **the runtime enters with a synchronous `with`, but its methods must be awaited; a Box enters with `async with`, and its methods must also be awaited — `info()` included.**
