...
<Note>
  Port forwarding maps a guest port onto the host running the sandbox, so it applies to boxes you run yourself. For how reaching a service differs on BoxLite Cloud, see [BoxLite Cloud vs open source](/cloud/vs-opensource).
</Note>
