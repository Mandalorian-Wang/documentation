...
| `box.info()` returns a coroutine, or awaiting it errors | `info()` is a coroutine on every type on this page, and synchronous only on the native `Box` | These types: `info = await box.info()`. Native `Box`: `box.info()` |
