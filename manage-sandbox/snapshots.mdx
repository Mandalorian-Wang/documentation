...
`info()` is synchronous on the native `Box`; on `SimpleBox` and its subclasses it is a coroutine, so write `info = await box.info()`. Either way it returns a `BoxInfo` whose state is at `info.state` (a `BoxStateInfo`, whose field is named `.status`).
