...
| Trigger | Condition |
|---|---|
| push to `main` | Changes to runtime code: `src/boxlite/**`, `src/shared/**`, `src/cli/**`, `src/guest/**`, `sdks/**`, any `**/Cargo.toml`, `Cargo.lock`, or the workflow file itself. |
| pull_request (`labeled` / `synchronize`) | Runs only when the PR carries the `e2e-local` label — the label is a cost gate that only maintainers can add. |
...
| AMI / region | Supplied by the workflow inputs `EC2_AMI_ID` / `AWS_REGION` |
...
| `make test:integration:sdk` | SDK integration suites (Python + Node + C; requires a VM). |
| `make test:integration:python` / `:node` / `:c` / `:rust` / `:cli` | Single-language / single-layer suites (some support `FILTER=<pattern>`). |
