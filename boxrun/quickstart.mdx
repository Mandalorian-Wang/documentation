...
For command details after installation, use [BoxRun CLI Reference](/boxrun/cli). This quickstart is only the first-run path.
...
Run `boxrun images` to see all recommended aliases. Any valid OCI image reference also works (e.g. `python:3.11`, `nginx:latest`).
...
The Python package name is `boxrun-sdk`, and the import path is `boxrun_sdk`.
...
<CardGroup cols={2}>
  <Card title="CLI Reference" icon="terminal" href="/boxrun/cli">
    Canonical BoxRun CLI page. Use it for supported commands, flags, upgrade, and uninstall.
  </Card>
