...
description: "Canonical BoxRun CLI reference for supported commands, flags, and lifecycle workflows."
---

This is the canonical BoxRun CLI reference.

Use this page as the single source of truth for `boxrun` commands. Older links to `/reference/cli` now exist only as a compatibility page and point back here.

`BOX` arguments accept either a box ID (`box_abc123`) or a name (`dev`).

## Command Summary

The following top-level commands are available in the local `boxrun` binary:

| Command | Purpose |
|---------|---------|
| `serve` | Start the BoxRun server |
| `create` | Create a new box |
| `ls` | List boxes |
| `stop` | Stop a running box |
| `start` | Start a stopped box |
| `rm` | Remove a box |
| `exec` | Execute a command in a box |
| `attach` | Attach an interactive terminal to a box |
| `shell` | Create a box and attach immediately |
| `cp` | Copy files between host and box |
| `run` | Run a command in an ephemeral box |
| `gc` | Remove old stopped boxes |
| `images` | List recommended images and aliases |
| `completion` | Generate shell completions |
| `upgrade` | Self-update to the latest release |
| `uninstall` | Remove `boxrun` from the system |
...
|-------|-------|-------------|
| `default` | `ubuntu:24.04` | Ubuntu 24.04 (general purpose, recommended) |
...
| `alpine` | `alpine:3.20` | Lightweight Linux (5MB) |
...
The CLI also accepts `elvish` and `powershell` as completion targets:

```bash
boxrun completion powershell
boxrun completion elvish
```
...
Update the local `boxrun` installation to the latest release.
...
Remove the local `boxrun` installation from the system.
